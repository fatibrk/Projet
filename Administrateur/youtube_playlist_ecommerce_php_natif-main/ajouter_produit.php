...
<div class="container py-2">
    ...
    if (isset($_POST['ajouter'])) {
        ...
        if (!empty($libelle) && !empty($prix) && !empty($categorie)) {
            ...
            if ($inserted) {
                // Redirection vers la page des produits avec les données du produit ajouté
                header('location: produits.php?libelle=' . urlencode($libelle) . '&prix=' . urlencode($prix) . '&categorie=' . urlencode($categorie));
                exit;
            } else {
                // Traitement en cas d'erreur d'insertion dans la base de données
                ?>
